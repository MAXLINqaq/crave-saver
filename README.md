...
- **AI 截图识别**：OpenAI 兼容多模态接口（默认 Qwen3-VL-30B-A3B，可换任意兼容服务），打包费/配送费/优惠后价都能正确入账；WorkManager 后台执行 + 完成通知
- **连续忍住天数**：正反馈对抗冲动消费
...
首页右上角进入设置页。Base URL 和 Model 已在 App 内预填一组开箱可用的默认值，API Key 填你自己的即可；三栏都可改，换成任何 OpenAI 兼容的多模态服务都行。
